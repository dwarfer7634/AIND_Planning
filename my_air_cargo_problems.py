...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
            ...
            for cargo in self.cargos : 
                for plane in self.planes : 
                    for airport in self.airports :
                        ...
                        effext_rem = [expr('In({}, {})'.format(cargo, plane))]
                        unload = Action(expr('Unload({}, {}, {})'.format(cargo, plane, airport)),
                                        [precond_pos, precond_neg],
                                        [effect_add, effext_rem])
        ...
        def fly_actions():
            ...
            """
            flys = []
            for from_ in self.airports:
                for to in self.airports:
                    if from_ != to:
                        for plane in self.planes:
                            precond_pos = [expr("At({}, {})".format(plane, from_)),
                                           ]
                            precond_neg = []
                            effect_add = [expr("At({}, {})".format(plane, to))]
                            effect_rem = [expr("At({}, {})".format(plane, from_))]
                            fly = Action(expr("Fly({}, {}, {})".format(plane, from_, to)),
                                         [precond_pos, precond_neg],
                                         [effect_add, effect_rem])
                            flys.append(fly)
            return flys
    ...
    def actions(self, state: str) -> list:
        ...
        #print('actions_list type: ',type(self.actions_list)) #data type of actions_list
        for action in self.actions_list :
            #print('action type: ', type(action)) #data type of action
            is_possible = True
